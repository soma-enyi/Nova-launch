...
const VALID_C_ADDRESS = 'CAAZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN';
const FACTORY_CONTRACT = VALID_C_ADDRESS;
const TX_HASH = 'abc123def456abc123def456abc123def456abc123def456abc123def456abcd';
...
describe('StellarService', () => {
  ...
  describe('getFactoryState', () => {
    it('throws when factory contract ID is not configured', async () => {
      ...
      svc.onModuleInit();
      await expect(svc.getFactoryState()).rejects.toThrow(StellarContractException);
    });
  ...
  describe('parseContractEvent', () => {
    ...
    it('throws StellarParseException on malformed event', () => {
      ...
      expect(() =>
        service.parseContractEvent({} as any),
      ).toThrow();
    });
  });
  ...
  describe('getTransaction', () => {
    ...
    it('throws for missing txHash', async () => {
      await expect(service.getTransaction('')).rejects.toThrow(StellarNotFoundException);
    });
  });

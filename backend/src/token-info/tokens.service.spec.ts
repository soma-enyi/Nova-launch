...
describe('TokensService', () => {
  ...
  describe('getToken - cache behavior', () => {
    ...
    it('should fetch from blockchain on cache miss', async () => {
      ...
      mockStellarService.parseAddress.mockReturnValue({ assetCode: 'USDC', assetIssuer: 'GABCDE' });
  ...
  describe('getToken - error handling', () => {
    ...
    it('should throw NotFoundException when asset not found on Stellar', async () => {
      mockStellarService.parseAddress.mockReturnValue({ assetCode: 'FAKE', assetIssuer: 'GABCDE' });
      mockStellarService.getAssetData.mockResolvedValue(null);

      await expect(service.getToken('FAKE:GABCDE')).rejects.toThrow(NotFoundException);
    });

    it('should throw BadRequestException for invalid address', async () => {
      mockStellarService.parseAddress.mockReturnValue({ assetCode: '', assetIssuer: '' });

      await expect(service.getToken('invalid')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('getToken - with includes', () => {
    ...
    it('should fetch analytics when include=analytics', async () => {
      ...
      expect(mockStellarService.getVolumeData).toHaveBeenCalledWith('USDC', 'GABCDE');
    ...
    it('should handle failed optional fetches gracefully', async () => {
      mockStellarService.getBurnStatistics.mockRejectedValue(new Error('Network error'));
      mockStellarService.getVolumeData.mockRejectedValue(new Error('Network error'));
    ...
    it('should cache result after successful fetch', async () => {
      const result = await service.getToken('USDC:GABCDE');
      expect(mockTokenCacheService.set).toHaveBeenCalledWith('cache-key', result.token);
    });
  });

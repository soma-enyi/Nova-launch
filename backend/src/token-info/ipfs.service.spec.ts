...
function makeResponse<T>(data: T): AxiosResponse<T> {
  return { data, status: 200, statusText: 'OK', headers: {}, config: { headers: {} } as any };
}
...
describe('IpfsService', () => {
  ...
  describe('resolveUrl', () => {
    it('should return http URLs as-is', () => {
      expect(service.resolveUrl('https://example.com/meta.json')).toBe('https://example.com/meta.json');
    });

    it('should convert ipfs:// URLs to gateway URL', () => {
      expect(service.resolveUrl('ipfs://QmHash123')).toBe('https://ipfs.io/ipfs/QmHash123');
    });

    it('should convert raw Qm... hash to gateway URL', () => {
      expect(service.resolveUrl('QmHash456')).toBe('https://ipfs.io/ipfs/QmHash456');
    });

    it('should convert raw bafy... hash to gateway URL', () => {
      expect(service.resolveUrl('bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi')).toBe(

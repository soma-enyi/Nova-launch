...
describe('StellarService', () => {
  ...
  describe('parseAddress', () => {
    ...
    it('should handle standalone address', () => {
      ...
      expect(result).toEqual({ assetCode: 'UNKNOWN', assetIssuer: 'GABCDE12345' });
    });
  });
  ...
  describe('getAssetData', () => {
    it('should return asset data when found', async () => {
      const mockAsset = {
        ...
        num_accounts: 5000,
        balances: { authorized: '900000', authorized_to_maintain_liabilities: '0', unauthorized: '0' },
      ...
      mockHttpService.get.mockReturnValue(
        of(
          makeResponse({
            _embedded: {
              records: [
                { type: 'create_account', created_at: '2022-01-01T00:00:00Z', source_account: 'GCREATOR' },
              ],
            },
          }),
        ),
  ...
  describe('getBurnStatistics', () => {
      ...
      mockHttpService.get.mockReturnValue(
        of(
          makeResponse({
            _embedded: {
              records: [
                { type: 'payment', asset_code: 'USDC', to: 'GABCDE', amount: '500.0000000' },
                { type: 'payment', asset_code: 'USDC', to: 'GABCDE', amount: '300.0000000' },
                { type: 'payment', asset_code: 'USDC', to: 'GOTHER', amount: '100.0000000' }, // not a burn
              ],
            },
          }),
        ),
    ...
    it('should return zero stats on error', async () => {
      mockHttpService.get.mockReturnValue(throwError(() => new Error('Error')));
      const result = await service.getBurnStatistics('USDC', 'GABCDE', '10000');
      expect(result).toEqual({ totalBurned: '0', burnCount: 0, percentBurned: '0.0000' });
    });
  });
      ...
      mockHttpService.get.mockReturnValue(
        of(
          makeResponse({
            _embedded: {
              records: [
                { base_amount: '1000.0000000', counter_amount: '2000', ledger_close_time: recentTime },
                { base_amount: '500.0000000', counter_amount: '1000', ledger_close_time: oldTime },
              ],
            },
          }),
        ),

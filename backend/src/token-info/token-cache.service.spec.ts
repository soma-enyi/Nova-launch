...
import { TokenCacheService, TOKEN_CACHE_PREFIX } from '../../src/tokens/token-cache.service';
...
const mockToken: Token = {
  basicInfo: { name: 'TEST', symbol: 'TEST', decimals: 7, address: 'TEST:GABCDE' },
...
describe('TokenCacheService', () => {
  ...
  describe('set', () => {
    it('should store token in cache with TTL', async () => {
      ...
      expect(mockCacheManager.set).toHaveBeenCalledWith('some-key', mockToken, 120000);
    });

    it('should use default TTL when not provided', async () => {
      mockCacheManager.set.mockResolvedValue(undefined);
      await service.set('some-key', mockToken);
      expect(mockCacheManager.set).toHaveBeenCalledWith('some-key', mockToken, 60000);
    });

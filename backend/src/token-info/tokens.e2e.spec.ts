...
const VALID_ADDRESS = 'USDC:GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN';

const mockToken: Token = {
  basicInfo: { name: 'USDC', symbol: 'USDC', decimals: 7, address: VALID_ADDRESS },
  supplyInfo: { total: '1000000.0000000', initial: '1000000.0000000', circulating: '900000.0000000' },
  burnInfo: { totalBurned: '1000.0000000', burnCount: 2, percentBurned: '0.1000' },
  creator: { address: 'GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN', createdAt: '2021-01-01T00:00:00Z' },
  analytics: { volume24h: '50000.0000000', volume7d: '350000.0000000', txCount24h: 120 },
...
describe('Tokens E2E Tests (/api/tokens/:address)', () => {
  ...
  afterEach(() => {
    jest.clearAllMocks();
    mockTokensService.getToken.mockResolvedValue({ token: mockToken, cached: false });
  });
  ...
  describe('GET /api/tokens/:address', () => {
    ...
    it('should accept include query params', async () => {
      ...
      expect(mockTokensService.getToken).toHaveBeenCalledWith(
        VALID_ADDRESS,
        expect.objectContaining({ include: expect.arrayContaining(['metadata', 'burns']) }),
      );
    });

    it('should return cached=true when response is from cache', async () => {
      mockTokensService.getToken.mockResolvedValue({ token: mockToken, cached: true });
    ...
    it('should return 404 when token not found', async () => {
      ...
      mockTokensService.getToken.mockRejectedValue(new NotFoundException('Token not found'));
    ...
    it('should include all three optional data sections when all are requested', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/tokens/${VALID_ADDRESS}?include=metadata&include=burns&include=analytics`)
        .expect(200);

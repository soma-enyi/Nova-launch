...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requiredRoles = this.reflector.getAllAndOverride<string[]>(DECORATORS.ROLES, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    const { user } = context.switchToHttp().getRequest<{ user: JwtPayloadDto & { roles?: string[] } }>();

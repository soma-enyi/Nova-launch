...
describe('JwtAuthGuard', () => {
  ...
  it('should throw UnauthorizedException when no user', () => {
    expect(() => guard.handleRequest(null, null, { message: 'No token' })).toThrow(
...
describe('RolesGuard', () => {
  ...
  it('should allow when user has required role', () => {
    ...
    const ctx = mockContext({ request: { user: { roles: ['admin', 'user'] } } });
    expect(guard.canActivate(ctx)).toBe(true);
  });
...
describe('ApiKeyGuard', () => {
        ...
        ApiKeyGuard,
        {
          ...
          useValue: {
            get: jest.fn((key: string, def: string) => {
              if (key === 'API_KEYS') return 'valid-api-key-123,another-key-456';
              return def;
            }),
          },
        },
  ...
  it('should allow valid API key from header', () => {
    const ctx = mockContext({
      request: { headers: { 'x-api-key': 'valid-api-key-123' }, query: {}, ip: '127.0.0.1' },
    });
    expect(guard.canActivate(ctx)).toBe(true);
  });
  ...
  it('should allow valid API key from query param', () => {
    const ctx = mockContext({
      request: { headers: {}, query: { api_key: 'another-key-456' }, ip: '127.0.0.1' },
    });
    expect(guard.canActivate(ctx)).toBe(true);
  });
  ...
  it('should throw for invalid key', () => {
    const ctx = mockContext({
      request: { headers: { 'x-api-key': 'wrong-key' }, query: {}, ip: '127.0.0.1' },
    });

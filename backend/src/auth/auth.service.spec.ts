...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    stellarSig = module.get(StellarSignatureService) as jest.Mocked<StellarSignatureService>;
  ...
  describe('requestNonce', () => {
    it('should return a nonce for valid public key', () => {
      const mockNonce = { nonce: 'abc', expiresAt: Date.now() + 5000, message: 'Sign...' };
    ...
    it('should throw BadRequestException for invalid public key', () => {
      ...
      expect(() => service.requestNonce('invalid')).toThrow(BadRequestException);
  ...
  describe('authenticateWithWallet', () => {
    ...
    it('should return token pair on valid authentication', async () => {
      ...
      stellarSig.verifySignature.mockReturnValue({ valid: true, publicKey: 'GTEST' });
      tokenService.generateTokenPair.mockReturnValue(mockTokenPair);

      const result = await service.authenticateWithWallet(validDto);

      expect(nonceService.consumeNonce).toHaveBeenCalledWith('valid-nonce', 'GTEST');
      expect(stellarSig.verifySignature).toHaveBeenCalledWith('GTEST', 'valid-sig', 'valid-nonce');
    ...
    it('should throw UnauthorizedException for invalid signature', async () => {
      stellarSig.isValidPublicKey.mockReturnValue(true);
      nonceService.consumeNonce.mockReturnValue(true);
      stellarSig.verifySignature.mockReturnValue({ valid: false, publicKey: 'GTEST', error: 'bad sig' });

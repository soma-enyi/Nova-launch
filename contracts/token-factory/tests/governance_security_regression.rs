...
fn setup() -> (Env, Address, Address, Address, TokenFactoryClient<'static>) {
    ...
    client.initialize(&admin, &treasury, &1_000_000, &500_000).unwrap();
...
fn test_unauthorized_schedule_fee_update() {
    ...
    client.schedule_fee_update(&attacker, &Some(999_999_999), &None).unwrap();
}
...
fn test_unauthorized_schedule_treasury_update() {
    ...
    client.schedule_treasury_update(&attacker, &new_treasury).unwrap();
}
...
fn test_unauthorized_cancel_change() {
    ...
    let change_id = client.schedule_fee_update(&admin, &Some(2_000_000), &None).unwrap();
...
fn test_cannot_bypass_timelock_by_canceling_and_requeuing() {
    ...
    let change_id_1 = client.schedule_fee_update(&admin, &Some(2_000_000), &None).unwrap();
    
    env.ledger().with_mut(|li| li.timestamp += 1800);
    
    client.cancel_change(&admin, &change_id_1).unwrap();
    let change_id_2 = client.schedule_fee_update(&admin, &Some(2_000_000), &None).unwrap();
...
fn test_timelock_enforced_for_treasury_update() {
    ...
    let new_treasury = Address::generate(&env);
    let change_id = client.schedule_treasury_update(&admin, &new_treasury).unwrap();
    client.execute_change(&change_id).unwrap();
}
...
fn test_schedule_fee_update_with_negative_base_fee() {
    ...
    client.schedule_fee_update(&admin, &Some(-1), &None).unwrap();
}
...
fn test_schedule_fee_update_with_negative_metadata_fee() {
    ...
    client.schedule_fee_update(&admin, &None, &Some(-1)).unwrap();
}
...
fn test_multiple_queued_changes_last_wins() {
    ...
    let change_id_1 = client.schedule_fee_update(&admin, &Some(2_000_000), &None).unwrap();
    let change_id_2 = client.schedule_fee_update(&admin, &Some(3_000_000), &None).unwrap();
    let change_id_3 = client.schedule_fee_update(&admin, &Some(4_000_000), &None).unwrap();
...
fn test_treasury_cannot_queue_changes() {
    ...
    client.schedule_fee_update(&treasury, &Some(2_000_000), &None).unwrap();
}
...
fn test_all_governance_functions_require_proper_auth() {
    ...
    assert!(client.try_schedule_fee_update(&attacker, &Some(2_000_000), &None).is_err());
    assert!(client.try_schedule_pause_update(&attacker, &true).is_err());
    
    let new_treasury = Address::generate(&env);
    assert!(client.try_schedule_treasury_update(&attacker, &new_treasury).is_err());
...
fn test_timelock_enforced_across_all_operations() {
    ...
    let fee_change = client.schedule_fee_update(&admin, &Some(2_000_000), &None).unwrap();
    let pause_change = client.schedule_pause_update(&admin, &true).unwrap();
    let new_treasury = Address::generate(&env);
    let treasury_change = client.schedule_treasury_update(&admin, &new_treasury).unwrap();
...
fn test_extreme_fee_values_accepted() {
    ...
    let change_id = client.schedule_fee_update(&admin, &Some(i128::MAX), &None).unwrap();
